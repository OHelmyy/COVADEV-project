...
function AccordionItem({ item, color, soft, border }: { item: HelpItem; color: string; soft: string; border: string }) {
        ...
        <div style={{ padding: "0 18px 16px", background: "#fafafa", borderTop: `1px solid ${border}` }}>
          ...
          {item.tags && (
            <div style={{ marginTop: 12, display: "flex", gap: 6, flexWrap: "wrap" }}>
              {item.tags.map(tag => (
                <span key={tag} style={{ fontSize: 11, fontWeight: 700, background: soft, color, border: `1px solid ${border}`, padding: "2px 9px", borderRadius: 999 }}>
                  #{tag}
                </span>
              ))}
            </div>
...
export default function HelpPage() {
  ...
  const sections = isDeveloper ? DEVELOPER_SECTIONS : isEvaluator ? EVALUATOR_SECTIONS : [];
  ...
  return (
    <div style={{
      minHeight: "100vh",
      background: "radial-gradient(circle at top right, rgba(6,182,212,0.07), transparent 22%), radial-gradient(circle at top left, rgba(109,40,217,0.05), transparent 18%), #f4f7fb",
      ...
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 24, position: "relative" }}>
          ...
          <input
            ...
            placeholder="Search for anything — submit, score, GitHub, AI…"
